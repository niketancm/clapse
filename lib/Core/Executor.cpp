...
static std::vector<int> parse(std::string l, char delim)
{
...
const Module *Executor::setModule(llvm::Module *module, 
  ...
  //save the basic block ids
	  for(Module::iterator f = module->begin(), fe=module->end();f!=fe;++f)
	  {
		  //if(f->size()>2)
		  {
		  Function::iterator BB = f->begin(), BE=f->end();
		  //BB++;//Jeff: skip the first BB in each function - entry
		  //BE--;//Jeff: skip the last BB in each function - return
		  for(;BB!=BE;++BB)
		  {
	  ...
	  //initialize threadbbiidsmap

	  std::ifstream file("bbiid");
	  ...
	  if(file)
	  {
		  ...
		  {
			  size_t tmp = str.find_first_of(" ",pos1);
			  std::string name = str.substr(pos1,tmp-pos1);
			  size_t start_pos = tmp+1;
			  std::string tstr = str.substr(start_pos,pos2-start_pos);
			  std::vector<int> bbvec = parse(tstr,' ');
			  threadbbiidsmap[name]=bbvec;

			  std::vector<int>::iterator it;
			  for(it=bbvec.begin();it<bbvec.end();it++)
			  {
				  BasicBlock *bb = basicblocks[*it];

				  num_instructions+= (int)bb->size();
			  }
			  num_threads++;
			  num_branches+=(int)bbvec.size();
		  ...
		  std::cerr << "NUM OF THREADS: "<<num_threads<<"\n";
		  std::cerr << "NUM OF BRANCHES: "<<num_branches<<"\n";
		  std::cerr << "NUM OF INSTRUCTIONS: "<<num_instructions<<"\n";
...
void Executor::printSyncTrace(std::string type)
{
	std::ostream &traceFile = interpreterHandler->getTraceStream();

	traceFile << "S-" << type<<"-"<<getThreadName()<<"\n";
			...
			for(std::map<const llvm::GlobalValue*, ref<ConstantExpr> > ::iterator iter = globalAddresses.begin();
				iter!=globalAddresses.end();iter++)
				{
					if(iter->second==e)
					{
						...
						{
							//llvm::errs()<<"Jeff: thread function name:"<<f->getName()<<"\n";
							...
							numOfUndefinedBB = 0;
							  ...
							  //run(state);
								...
								//std::cerr << "Thread "<<tname<<" terminated\n";
...
//							    Instruction *i = ki->inst;
//
//							    llvm::errs()<<"Jeff: Child OpCode "<<i->getOpcode()<<"\n";
//							      switch (i->getOpcode()) {
//
//							      }
						}
	...
	//				  		uint64_t value = e.getConstantValue();
	//
	//				  		printf("Jeff: thread function name?? %lld\n",value);
	//				  		if (Function *f = dyn_cast<Function*>(value))
	//				  		{
	//					  		printf("Jeff: YES thread function name?? %lld\n",value);
	//
	//				  		}
					}
				}
...
void Executor::newBasicBlockEntry(ExecutionState &state, std::vector< ref<Expr> > &arguments)
{
	...
	//std::cerr << "Hi:"<<tname<<"-" << iid <<"-"<< expr<< "\n";

	int fail = expr.compare(ConstantExpr::create(iid, 32));
	if(fail)
	{
		int size = states.size()+addedStates.size();

		//std::cerr << "NO("<<size<<"):"<<getThreadName()<<"-" <<iid<<"-"<< expr << "\n";
...
//	    llvm::errs() << *(state.pc->inst) <<"\n";
	...
	else
	{
		...
		//std::cerr << "YES("<<size<<"):"<<getThreadName()<<"-" << iid << "\n";
		if(size>0)
...
void Executor::transferToBasicBlock(BasicBlock *dst, BasicBlock *src, 
	...
	//Jeff: basic block enforcement
	//Maybe we should not be here

	//if all basic blocks are executed, just terminate
	if(threadbbiidsmap[getThreadName()].size()==0)
	{
		std::cerr << "OH, ALL BASICBLOCKS ARE DONE, LET'S TERMINATE.\n";
		terminateStateCLAPWithConstraints(state);
		exit(0);
	}
/*
	int iid = threadbbiidsmap[tname].back();
	//std::cerr << "Jeff: thread "<<tname<<" In transfer to basic block: "<<iid<<"\n";

	if(dst==basicblocks[iid])
	{
		std::cerr << "YES:"<<tname<<"-" << iid << "\n";
		threadbbiidsmap[tname].pop_back();

	}
	else
	{
		//if the dst basicblock is not the one, just return

		std::vector<llvm::BasicBlock*>::iterator iter = std::find(basicblocks.begin(),basicblocks.end(),dst);
		if(iter!=basicblocks.end())
		{
			size_t iid2 = std::distance(basicblocks.begin(),iter);

			//iid2 - the id of the dst basic block
			//iid2 is not the same as iid - the correct one

			if(addedStates.size()>0)
			{
				//terminate the exploration of the basic block
				terminateStateDirectly(state);
				std::cerr << "NOX:"<<tname<<"-"<<iid2<< "\n";
			...
			else
			{
				...
				//might because that some basic block we recorded at online is optimized away in KLEE
				//so we remove the basic block

				std::vector<int>::reverse_iterator iter2 = std::find(threadbbiidsmap[tname].rbegin(),threadbbiidsmap[tname].rend(),iid2);
				if(iter2!=threadbbiidsmap[tname].rend())
				{
					//YES
					//remove all incorrect basic blocks
					while(threadbbiidsmap[tname].back() !=iid2)
					{
						threadbbiidsmap[tname].pop_back();
					}
					threadbbiidsmap[tname].pop_back();
					std::cerr << "NO:"<<tname<<"-"<<iid2<<"-"<<iid<< "\n";
				}
			...
			//should not exceed ten, just terminate
			if(numOfUndefinedBB++>10&&tname!="0")
				terminateState(state);

		}
		else
		{
			//this basic block is not in our basicblock sequence, how come?! inlined?
			//could be __cxx_global_var_init,,. internal linkage

			std::cerr << tname <<": WE DON'T HAVE THIS DST BASICBLOCK.\n";

			//let's print all the instructions of this basic block
		    //llvm::errs() << *(state.pc->inst) <<"\n";

		    //dst->dump();

//		    llvm::BasicBlock::InstListType::iterator it;
//		    for(it = dst->getInstList().begin();it < dst->getInstList().end();it++)
//		    {
//		    	llvm::errs() << *it <<"\n";
//		    }
//
...
//		int iid2=0;
//
//		for (unsigned int i=0; i<threadbbiidsmap[tname].size(); i++)
//		{
//			iid2 = threadbbiidsmap[tname][i];
//			if(dst==basicblocks[iid2])
//				break;
//		}
//
//		//terminate state early
//		//terminateStateEarly(state, "clap - wrong path");
//
//		//terminateState(state);
//
//		if(addedStates.size()>0)
...
//			std::cerr << "Jeff: NO basic block "<<iid2<< "\n";
...
//nikni337
void Executor::allocateGlobalMemory(ExecutionState &state)
{
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Ret: {
    ...
    if (state.stack.size() <= 1) {
      ...
      //std::cerr << "Jeff: execution terminate\n";
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      //std::cerr <<"Thread "<<tname<< "-Branch Condition: "<<cond<<"\n";
...
//        statsTracker->markBranchVisited(branches.first, branches.second);
      ...
      //Jeff: need to decide which one to go here

      //TODO: JEFF JUST DISABLE ONE FOR TESTING
      if (branches.first)
      {
        transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *branches.first);
        //std::cerr << "Jeff: go branch first\n";
      }
      ...
      if (branches.second)
      {
        ...
        //std::cerr << "Jeff: go branch second\n";
      }
...
/*
		std::string name =  varaddrnamemap[base];


		if(name.length())
		{
			std::cerr <<tname<<" - Load: "<<base<<"  - Name: "<<name<<"\n";

			  Executor::ExactResolutionList rl;

			  resolveExact(state, base, rl, "clap_load_make_symbolic");
			  ...
			  for (Executor::ExactResolutionList::iterator it = rl.begin(),
			    ...
			    std::string uniqueName = executeMakeSymbolic(*s, mo, name);
			    ...
			    traceFile.flush();
				  ...
				  //std::cerr << "Hey, i'm here read ~~ "<<name<<"\n";

//			for (unsigned i = 0; i != state.symbolics.size(); ++i) {
//				    const MemoryObject *mo = state.symbolics[i].first;
//
//				        	  if(mo->getBaseExpr()==er)
//				        	  {
//				        		  const Array *arr = state.symbolics[i].second;
//				        	    	std::ostream &traceFile = interpreterHandler->getTraceStream();
//				        	    	traceFile << "R-" << arr->name<<"\n";
//
//				        		  break;
//				        	  }
//				    }
  ...
  case Instruction::Store: {
    ...
    state.addressSpace.resolve(state,solver,base,rl);
	  ...
	  for (ResolutionList::iterator it = rl.begin(),
		...
		if(name.length())
		{
			//std::cerr <<tname<<" - Store: "<<base<<"  - Name: "<<name<<" - "<<value<<"\n";
			...
			std::ostream &traceFile = interpreterHandler->getTraceStream();
							  traceFile << "W-" << name<<"_"<<basevalue<<"-"<<getThreadName()<<"\n";
								if(!isa<ConstantExpr>(base))
									traceFile <<"*"<<base<<"*\n";

								traceFile <<"$"<<value<<"$\n";

//								if(Expr *e = dyn_cast<Expr>(value))
//										 {
//										e->print(traceFile);
//										traceFile << "\n";
//										 }
...
void Executor::terminateStateCLAPWithConstraints(ExecutionState &state) {
	...
	interpreterHandler->incPathsExplored();
	  ...
	  stateTerminate = true;
		//std::cerr<<"Thread "<<getThreadName()<<" terminated\n";
