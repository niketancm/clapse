...
#define addDNR(name, handler) { name, \
  ...
  //myBasicBlockEntryi
  add("_Z17myBasicBlockEntryi", handleBasicBlockEntry,false),
  ...
  add("pthread_cond_wait", handlePThreadWait, false),
  add("pthread_cond_timedwait", handlePThreadTimedwait, false),
  add("\01_pthread_cond_timedwait", handlePThreadTimedwait, false),

  add("pthread_cond_signal", handlePThreadSignal, false),
...
void SpecialFunctionHandler::handleMarkGlobal(ExecutionState &state,
  ...
  void SpecialFunctionHandler::handlePThreadCreate(ExecutionState &state,
          ...
          std::vector<ref<Expr> > &arguments)
  {
	  //std::cerr << "Jeff: handling pthread_create\n";
	  ...
	  //fork a new process
		...
		{ // This is the child process
			...
			std::stringstream ss;//create a stringstream
						   ss << executor.tid;//add number to the stream

			if(getThreadName()=="0")
				setThreadName(ss.str());//return a string with the contents of the stream
			else
				setThreadName(getThreadName()+"_"+ss.str());


			executor.tid=1;

			//std::cerr << "Child Thread "<<executor.tname<<"\n";
			//GO TO HANDLE THE NEW THREAD
		...
		{ // This is the parent process
			executor.tid++;
			//printf("Parent\n");
			//std::cerr << "Parent Thread "<<executor.tname<<"\n";

			  std::string str = "fork";
			  executor.printSyncTrace(str);
		}
  }
  ...
  void SpecialFunctionHandler::handlePThreadTimedwait(ExecutionState &state,
          ...
          std::vector<ref<Expr> > &arguments)
  {
	  std::string str = "timedwait_unlock";
	  executor.printSyncTrace(str);
	  str = "timedwait_lock";
	  executor.printSyncTrace(str);
